...
class WorkTimer(QWidget):
    ...
    def save_records(self):
        if not self.records:
            # 无记录时，在时间标签上闪一下“⚠ 无记录”（无声）
            self.time_label.setText("⚠ 无记录")
            QTimer.singleShot(1500, lambda: self.time_label.setText("00:00:00" if not self.is_running else self.time_label.text()))
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "保存记录", "工作时间记录.xlsx", "Excel文件 (*.xlsx)")
        if not file_path:
            # 用户取消了保存对话框，静默返回
            return
        try:
            ...
            # ---------- 设置每一列的宽度（保留你原来的数值） ----------
            column_widths = [10, 22, 22, 10]   # 单位：字符宽度
            ...
            wb.save(file_path)
            # ✅ 保存成功 → 时间标签显示“✔ 已保存”1.5秒（无声）
            self.time_label.setText("✔ 已保存")
            QTimer.singleShot(1500, lambda: self.time_label.setText("00:00:00" if not self.is_running else self.time_label.text()))
        except Exception as e:
            # ❌ 保存失败 → 时间标签显示“✘ 保存失败”1.5秒（无声）
